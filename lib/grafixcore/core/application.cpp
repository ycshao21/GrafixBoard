...
// newer binary of GLFW that is adequate for your version of Visual Studio. #if
...
// // Unlike g_MainWindowData.FrameIndex, this is not the the swapchain image
// index
...
// static VKAPI_ATTR VkBool32 VKAPI_CALL DebugReport(VkDebugReportFlagsEXT
// flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t
// location, int32_t messageCode, const char* pLayerPrefix, const char*
// pMessage, void* pUserData)
// {
//     GF_TRACE("Vulkan debug report from ObjectType: {0}, message: {1}",
...
//     result = vkAcquireNextImageKHR(g_Device, wd->Swapchain, UINT64_MAX,
//     imageAcquiredSemaphore, VK_NULL_HANDLE, &wd->FrameIndex); if (result ==
...
//     {
//         result = vkWaitForFences(g_Device, 1, &fd->Fence, VK_TRUE,
...
//     // These use g_MainWindowData.FrameIndex and not s_CurrentFrameIndex
//     because they're tied to the swapchain image index
//     {
...
//             allocatedCommandBuffers.data()); allocatedCommandBuffers.clear();
//         }
...
//     drawData->DisplaySize.y <= 0.0f); wd->ClearValue.color.float32[0] =
//     clearColor.x * clearColor.w; wd->ClearValue.color.float32[1] =
//     clearColor.y * clearColor.w; wd->ClearValue.color.float32[2] =
//     clearColor.z * clearColor.w; wd->ClearValue.color.float32[3] =
//     clearColor.w; ImGui_ImplVulkan_RenderDrawData(drawData,
//     fd->CommandBuffer);
...
//         VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT; VkSubmitInfo info =
...
//     }
//     CheckVkResult(result);
//     wd->SemaphoreIndex = (wd->SemaphoreIndex + 1) % wd->ImageCount; // Now we
//     can use the next set of semaphores
// }
...
Application::Application(const ApplicationConfig& config)
{
    ...
    (void) config;
    GB_CORE_INFO("Application created.");

    // m_Window =
    //     std::make_unique<Window>(config.Name, config.Width,
    //     config.Height);
...
Application::~Application()
{
    ...
    GB_CORE_INFO("Application shutdown.");
}
...
{
    m_running = true;
    GB_CORE_INFO("Application running...");
    ...
    //             // pool is destroyed s_CommandBuffers.clear();
...
//         GF_ASSERT(vkCreateDebugReportCallbackEXT, "Cannot load
//         vkCreateDebugReportCallbackEXT!");

//         VkDebugReportCallbackCreateInfoEXT debugReportInfo = {};
//         debugReportInfo.sType =
//         VK_STRUCTURE_TYPE_DEBUG_REPORT_CALLBACK_CREATE_INFO_EXT;
//         debugReportInfo.flags = VK_DEBUG_REPORT_ERROR_BIT_EXT |
//         VK_DEBUG_REPORT_WARNING_BIT_EXT |
//         VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT;
//         debugReportInfo.pfnCallback = DebugReport;
//         debugReportInfo.pUserData = nullptr;

//         vkCreateDebugReportCallbackEXT(g_Instance, &debugReportInfo, nullptr,
//         &g_DebugReport);
// #endif // VULKAN_DEBUG_REPORT
//     }
...
//         GF_ASSERT(gpuCount > 0, "Cannot find physical devices with Vulkan
//         support!");
...
//         or use first one available. This covers
//         // most common cases (multi-gpu/integrated+dedicated graphics).
//         Handling more complicated setups (multiple
...
//         nullptr); std::vector<VkQueueFamilyProperties> queues(count);
//         vkGetPhysicalDeviceQueueFamilyProperties(g_PhysicalDevice, &count,
//         queues.data());
...
//         GF_ASSERT(g_QueueFamily != (uint32_t)-1, "Cannot find queue
//         families!");
//     }
...
//         sizeof(queueCreateInfo[0]); createInfo.pQueueCreateInfos =
//         queueCreateInfo; createInfo.enabledExtensionCount =
//         (uint32_t)deviceExtensions.size(); createInfo.ppEnabledExtensionNames
//         = deviceExtensions.data();

//         result = vkCreateDevice(g_PhysicalDevice, &createInfo, nullptr,
//         &g_Device); GF_ASSERT(result == VK_SUCCESS, "Cannot create logical
//         device!");
...
//         result = vkCreateDescriptorPool(g_Device, &poolInfo, nullptr,
//         &g_DescriptorPool); GF_ASSERT(result == VK_SUCCESS, "Failed to create
//         descriptor pool!");
//     }
...
//         result = glfwCreateWindowSurface(g_Instance, m_Window->GetHandle(),
//         nullptr, &g_Surface); GF_ASSERT(result == VK_SUCCESS, "Failed to
...
//         wd->Surface, &res); GF_ASSERT(res == VK_TRUE, "No WSI support on
//         physical device!");
...
//         ImGui_ImplVulkanH_SelectSurfaceFormat(g_PhysicalDevice, wd->Surface,
...
//         g_Device, wd, g_QueueFamily, nullptr, width, height,
//         g_MinImageCount);
//     }
...
//         VkCommandPool commandPool = wd->Frames[wd->FrameIndex].CommandPool;
//         VkCommandBuffer& commandBuffer =
//         s_CommandBuffers[wd->FrameIndex].emplace_back();
...
//             &commandBuffer); GF_ASSERT(result == VK_SUCCESS, "Cannot allocate
//             command buffer!");
//         }
...
//             GF_ASSERT(result == VK_SUCCESS, "Failed to begin command
//             buffer!");
//         }
